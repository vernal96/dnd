...
import { ArrowLeft, Crosshair, Package, Play, Swords, UserRound, Waves } from 'lucide-vue-next';
...
const isRuntimeApplying = ref(false);
...
const activeTerrain = ref<SceneSurfaceDefinition['code'] | null>(null);
const activeNpcSourceId = ref<number | null>(null);
const activeHeroSourceId = ref<number | null>(null);
const activeItemCode = ref<string | null>(null);
...
const isEncounterStarting = ref(false);
...
const sceneName = computed(() => sceneTemplate.value?.name ?? 'Runtime сцена');
const selectedActor = computed<RuntimeActorInstance | null>(() => {
  if (selectedActorId.value === null) {
    return null;
  }

  return runtimeScene.value?.actor_instances.find((actor) => actor.id === selectedActorId.value) ?? null;
});
...
const currentActorRemainingMovement = computed(() => currentEncounterParticipant.value?.movement_left ?? null);
const canLaunchEncounter = computed(() => (runtimeScene.value?.actor_instances.length ?? 0) > 0);
const runtimeActionLog = computed<Array<Record<string, unknown>>>(() => runtimeScene.value?.runtime_state?.action_log ?? []);
const availablePlayerCharacters = computed(() =>
  (gameDetail.value?.members ?? [])
    .filter((member) => member.role === 'player' && member.player_character !== null)
    .map((member) => ({
      memberId: member.id,
      playerCharacter: member.player_character!,
      user: member.user,
    })),
);
const selectedCell = computed<SceneCell | null>(() => {
  if (selectedCellKey.value === null || sceneTemplate.value === null) {
    return null;
  }
  ...
  return sceneTemplate.value.cells.find((cell) => cell.x === x && cell.y === y) ?? null;
});
const selectedActorInventory = computed<RuntimeActorInventoryItem[]>(() => normalizeInventory(selectedActor.value?.runtime_state?.inventory));
...
function syncEncounterSelection(): void {
  ...
  if (currentEncounterActor.value !== null) {
    selectedActorId.value = currentEncounterActor.value.id;

    if (currentEncounterActor.value.x !== null && currentEncounterActor.value.y !== null) {
      selectedCellKey.value = resolveCellKey(currentEncounterActor.value.x, currentEncounterActor.value.y);
    }
  }
}

function openEncounterStartModal(): void {
  selectedEncounterActorIds.value = (runtimeScene.value?.actor_instances ?? []).map((actor) => actor.id);
  isEncounterStartModalOpen.value = true;
}

function closeEncounterStartModal(): void {
  isEncounterStartModalOpen.value = false;
}

function toggleEncounterActor(actorId: number): void {
  if (selectedEncounterActorIds.value.includes(actorId)) {
    selectedEncounterActorIds.value = selectedEncounterActorIds.value.filter((id) => id !== actorId);
    return;
  }

  selectedEncounterActorIds.value = [...selectedEncounterActorIds.value, actorId];
}
...
async function handleEquipInventoryItem(slot: string, itemCode: string | null): Promise<void> {
  ...
  try {
    ...
    runtimeError.value = '';
    runtimeScene.value = await equipRuntimeActor(gameId.value, inventoryActor.value.id, {
      item_code: itemCode,
      slot,
    });
  } catch (error) {
...
async function handleWeaponAttack(targetActorId: number, equipmentSlot: string | null = null): Promise<void> {
  ...
  try {
    ...
    runtimeError.value = '';
    const updatedScene = await performRuntimeAction(gameId.value, selectedActor.value.id, {
...
function resolveInventoryItemName(itemCode: string): string {
  const catalogItem = itemCatalog.value.find((item) => item.code === itemCode);

  return catalogItem?.name ?? prettifyItemCode(itemCode);
}
...
function drawItemDrop(
  ...
  itemDrop: RuntimeSceneDetail['item_drops'][number],
): void {
  const baseX = projectedCell.center.x;
...
function drawSceneEntities(context: CanvasRenderingContext2D, projectedCells: ProjectedCell[]): void {
  ...
  for (const placement of placements) {
    ...
    if (placement.itemDrop) {
      drawItemDrop(context, placement.projectedCell, placement.itemDrop);
    }
  }
...
function handleCanvasContextMenu(event: MouseEvent): void {
  ...
  selectedActorId.value = selectedActorId.value ?? actorAtCell.id;
...
function canActorMoveNow(actor: RuntimeActorInstance): boolean {
  ...
  return currentEncounterParticipant.value?.actor_id === actor.id;
}
...
async function handleCellClick(x: number, y: number): Promise<void> {
  ...
  if (await applyRuntimeToolAtCell(x, y)) {
    return;
  }
  ...
  try {
    ...
    const updatedActor = await moveRuntimeActor(gameId.value, movingActor.id, { x, y });
...
function clearRuntimeTools(): void {
  activeTerrain.value = null;
  activeNpcSourceId.value = null;
  activeHeroSourceId.value = null;
  activeItemCode.value = null;
}

async function handleStartEncounter(): Promise<void> {
  if (gameId.value === null) {
    return;
  }

  try {
    isEncounterStarting.value = true;
    runtimeError.value = '';
    runtimeScene.value = await startRuntimeEncounter(gameId.value, {
      actor_ids: selectedEncounterActorIds.value,
    });
    closeEncounterStartModal();
    syncEncounterSelection();
    scheduleCanvasRender();
  } catch (error) {
    runtimeError.value = (error as Error).message;
  } finally {
    isEncounterStarting.value = false;
  }
}

async function consumeEncounterAction(actionType: 'action' | 'bonus-action'): Promise<void> {
  if (gameId.value === null || currentEncounterActor.value === null) {
    return;
  }
  ...
  try {
    ...
    runtimeScene.value = actionType === 'action'
      ? await useRuntimeAction(gameId.value, currentEncounterActor.value.id)
      : await useRuntimeBonusAction(gameId.value, currentEncounterActor.value.id);
    syncEncounterSelection();
    scheduleCanvasRender();
  } catch (error) {
    runtimeError.value = (error as Error).message;
...
async function handleEndEncounterTurn(): Promise<void> {
  ...
  try {
    isEncounterUpdating.value = true;
    runtimeError.value = '';
    runtimeScene.value = await endRuntimeTurn(gameId.value);
    syncEncounterSelection();
    scheduleCanvasRender();
  } catch (error) {
    runtimeError.value = (error as Error).message;
...
async function applyRuntimeToolAtCell(x: number, y: number): Promise<boolean> {
  if (gameId.value === null) {
    return false;
  }

  try {
    isRuntimeApplying.value = true;
    runtimeError.value = '';

    if (activeTerrain.value !== null) {
      runtimeScene.value = await paintRuntimeCell(gameId.value, {
        terrain_type: activeTerrain.value,
        x,
        y,
      });
      selectedCellKey.value = resolveCellKey(x, y);
      pushToast('Поверхность изменена', `Клетка ${x},${y} обновлена.`, 'success');
      return true;
    }

    if (activeNpcSourceId.value !== null) {
      runtimeScene.value = await spawnRuntimeActor(gameId.value, {
        source_type: 'npc',
        source_id: activeNpcSourceId.value,
        x,
        y,
      });
      selectedCellKey.value = resolveCellKey(x, y);
      clearRuntimeTools();
      pushToast('NPC добавлен', `NPC размещен в клетке ${x},${y}.`, 'success');
      return true;
    }

    if (activeHeroSourceId.value !== null) {
      runtimeScene.value = await spawnRuntimeActor(gameId.value, {
        source_type: 'player_character',
        source_id: activeHeroSourceId.value,
        x,
        y,
      });
      selectedCellKey.value = resolveCellKey(x, y);
      clearRuntimeTools();
      pushToast('Герой добавлен', `Герой игрока размещен в клетке ${x},${y}.`, 'success');
      return true;
    }

    if (activeItemCode.value !== null) {
      runtimeScene.value = await dropRuntimeItem(gameId.value, {
        item_code: activeItemCode.value,
        quantity: 1,
        x,
        y,
      });
      selectedCellKey.value = resolveCellKey(x, y);
      clearRuntimeTools();
      pushToast('Предмет размещен', `Предмет добавлен в клетку ${x},${y}.`, 'success');
      return true;
    }
  } catch (error) {
    runtimeError.value = (error as Error).message;
...
<template>
  ...
  >
    ...
    <div class="fixed left-6 top-6 z-30 flex flex-wrap items-center gap-3">
      <button
        ...
        type="button"
        @click="router.push(sceneBackLink)"
      >
      ...
      >
        <section class="scene-runtime-shell">
          <div class="scene-runtime-hintbar">
            ...
            <span>{{ sceneName }}</span>
            <span>ЛКМ: выбор персонажа или перемещение</span>
          ...
          <div class="scene-runtime-panel-scroll">
            <section class="rounded-[1.5rem] border border-amber-200/10 bg-slate-950/30 p-4">
              ...
              <div class="mt-4 flex flex-wrap gap-2 text-xs uppercase text-slate-400">
                ...
                <span class="rounded-full border border-amber-200/10 bg-white/5 px-3 py-1">
                  Версия {{ runtimeScene.version }}
                </span>
              </div>
              <div class="mt-4">
                <button
                  class="inline-flex items-center gap-2 rounded-full border border-amber-300/30 bg-amber-300/10 px-4 py-2 text-sm text-amber-50 transition hover:border-amber-200/40 disabled:cursor-not-allowed disabled:opacity-50"
                  :disabled="!canLaunchEncounter || isEncounterStarting"
                  type="button"
                  @click="openEncounterStartModal()"
                >
                  <Swords class="h-4 w-4" />
                  {{ activeEncounter ? 'Перезапустить сражение' : 'Запустить сражение' }}
                </button>
              </div>
            </section>
            ...
            >
              ...
              <div class="mt-4 grid gap-2 text-sm text-slate-300">
                <div class="rounded-2xl border border-amber-200/10 bg-white/5 px-4 py-3">
                  Осталось перемещения: {{ currentActorRemainingMovement ?? '—' }} клеток
                </div>
                <div class="rounded-2xl border border-amber-200/10 bg-white/5 px-4 py-3">
                  Основное действие: {{ currentEncounterParticipant?.action_available ? 'доступно' : 'израсходовано' }}
                </div>
                <div class="rounded-2xl border border-amber-200/10 bg-white/5 px-4 py-3">
                  Доп. действие: {{ currentEncounterParticipant?.bonus_action_available ? 'доступно' : 'израсходовано' }}
                </div>
              </div>
              ...
              <div class="mt-4 grid gap-2">
                <button
                  ...
                  :disabled="!currentEncounterParticipant?.action_available || isEncounterUpdating"
                  type="button"
                ...
                <button
                  class="rounded-full border border-amber-200/20 bg-white/5 px-4 py-3 text-sm text-amber-50 transition hover:border-amber-200/35 disabled:cursor-not-allowed disabled:opacity-50"
                  :disabled="!currentEncounterParticipant?.bonus_action_available || isEncounterUpdating"
                  type="button"
            ...
            <section class="rounded-[1.5rem] border border-amber-200/10 bg-slate-950/30 p-4">
              ...
              <div class="mt-4 space-y-3">
                ...
                >
                  Выбери карточку персонажа на поле, затем кликни по целевой клетке.
                </div>

                <div
                  v-else
                  class="rounded-2xl border border-amber-200/10 bg-white/5 px-4 py-4"
                >
                  <div class="flex items-center gap-3">
                    <img
                      v-if="selectedActor.image_url"
                      :src="selectedActor.image_url"
                      :alt="selectedActor.name"
                      class="h-16 w-12 rounded-xl border border-white/10 object-cover object-top"
                    >
                    <div
                      v-else
                      class="flex h-16 w-12 items-center justify-center rounded-xl border border-white/10 bg-white/10 text-sm font-semibold text-amber-100"
                    >
                  ...
                  <div class="mt-4 rounded-2xl border border-amber-200/10 bg-slate-950/40 px-4 py-3 text-xs leading-5 text-slate-300">
                    {{ isActorMoving ? 'Перемещение выполняется...' : 'Следующий клик по свободной клетке отправит персонажа туда с анимацией. ПКМ по актеру открывает контекстное меню.' }}
                  </div>
                  <div
            ...
            <section class="rounded-[1.5rem] border border-amber-200/10 bg-slate-950/30 p-4">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <p class="text-xs uppercase tracking-[0.2em] text-amber-200/50">Инструменты</p>
                  <p class="mt-1 text-sm text-slate-300">
                    Во время игры ГМ может менять поверхность и добавлять сущности прямо на сцену.
                  </p>
                </div>
              </div>

              <div class="mt-4 grid gap-3">
                <div class="rounded-2xl border border-amber-200/10 bg-white/5 p-3">
                  <p class="text-xs uppercase text-amber-200/50">Поверхности</p>
                  <div class="mt-3 flex flex-wrap gap-2">
                    <button
                      v-for="surface in surfaceCatalog"
                      :key="surface.code"
                      :class="activeTerrain === surface.code ? 'border-amber-300/40 bg-amber-300/10 text-amber-50' : 'border-amber-200/10 bg-slate-950/40 text-slate-200'"
                      class="rounded-full border px-3 py-2 text-xs uppercase transition"
                      type="button"
                      @click="activeTerrain = activeTerrain === surface.code ? null : surface.code; activeNpcSourceId = null; activeHeroSourceId = null; activeItemCode = null"
                    >
                      {{ surface.name }}
                    </button>
                  </div>
                ...
                <div class="rounded-2xl border border-amber-200/10 bg-white/5 p-3">
                  <p class="text-xs uppercase text-amber-200/50">NPC</p>
                  <div class="mt-3 grid gap-2">
                    <button
                      v-for="actor in gameActors"
                      :key="actor.id"
                      :class="activeNpcSourceId === actor.id ? 'border-amber-300/40 bg-amber-300/10' : 'border-amber-200/10 bg-slate-950/40'"
                      class="flex items-center gap-3 rounded-2xl border p-3 text-left transition"
                      type="button"
                      @click="activeNpcSourceId = activeNpcSourceId === actor.id ? null : actor.id; activeTerrain = null; activeHeroSourceId = null; activeItemCode = null"
                    >
                      <img
                        v-if="actor.image_url"
                        :src="actor.image_url"
                        :alt="actor.name"
                        class="h-12 w-10 rounded-xl border border-white/10 object-cover object-top"
                      >
                      <div v-else class="flex h-12 w-10 items-center justify-center rounded-xl border border-white/10 bg-white/10 text-sm font-semibold text-amber-100">
                ...
                <div class="rounded-2xl border border-amber-200/10 bg-white/5 p-3">
                  <p class="text-xs uppercase text-amber-200/50">Герои игроков</p>
                  <div class="mt-3 grid gap-2">
                    <button
                      v-for="entry in availablePlayerCharacters"
                      :key="entry.playerCharacter.id"
                      :class="activeHeroSourceId === entry.playerCharacter.id ? 'border-amber-300/40 bg-amber-300/10' : 'border-amber-200/10 bg-slate-950/40'"
                      class="flex items-center gap-3 rounded-2xl border p-3 text-left transition"
                      type="button"
                      @click="activeHeroSourceId = activeHeroSourceId === entry.playerCharacter.id ? null : entry.playerCharacter.id; activeTerrain = null; activeNpcSourceId = null; activeItemCode = null"
                    >
                      <img
                        v-if="entry.playerCharacter.image_url"
                        :src="entry.playerCharacter.image_url"
                        :alt="entry.playerCharacter.name"
                        class="h-12 w-10 rounded-xl border border-white/10 object-cover object-top"
                      >
                      <div v-else class="flex h-12 w-10 items-center justify-center rounded-xl border border-white/10 bg-white/10 text-sm font-semibold text-amber-100">
                        {{ entry.playerCharacter.name.slice(0, 1) }}
                      </div>
                      <div class="min-w-0">
                        <span class="block truncate text-sm text-amber-50">{{ entry.playerCharacter.name }}</span>
                        <span class="block truncate text-xs text-slate-300">Игрок: {{ entry.user.name }}</span>
                      </div>
                    </button>
                  </div>
                </div>
                ...
                <div class="rounded-2xl border border-amber-200/10 bg-white/5 p-3">
                  <div class="flex items-center gap-2">
                    <Package class="h-4 w-4 text-amber-100" />
                    <p class="text-xs uppercase text-amber-200/50">Предметы</p>
                  </div>
                  <div class="mt-3 grid gap-2">
                    <button
                      v-for="item in itemCatalog.slice(0, 14)"
                      :key="item.code"
                      :class="activeItemCode === item.code ? 'border-amber-300/40 bg-amber-300/10' : 'border-amber-200/10 bg-slate-950/40'"
                      class="flex items-center gap-3 rounded-2xl border px-3 py-2 text-left text-sm text-slate-200 transition"
                      type="button"
                      @click="activeItemCode = activeItemCode === item.code ? null : item.code; activeTerrain = null; activeNpcSourceId = null; activeHeroSourceId = null"
                    >
                      <img
                        v-if="item.image_url"
                        :src="item.image_url"
                      ...
                      <div class="min-w-0">
                        ...
                        <span class="block truncate text-xs text-slate-400">{{ item.code }}</span>
                      </div>
                    </button>
                  </div>
                </div>
            ...
            <section class="rounded-[1.5rem] border border-amber-200/10 bg-slate-950/30 p-4">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <p class="text-xs uppercase tracking-[0.2em] text-amber-200/50">Персонажи</p>
                  <p class="mt-1 text-sm text-slate-300">{{ runtimeScene.actor_instances.length }} на активной сцене</p>
                </div>
              </div>

              <div class="mt-4 grid gap-3">
                <button
                  v-for="actor in runtimeScene.actor_instances"
                  :key="actor.id"
                  :class="selectedActorId === actor.id ? 'border-amber-300/40 bg-amber-300/10' : 'border-amber-200/10 bg-white/5'"
                  class="flex items-center gap-3 rounded-2xl border p-3 text-left transition hover:border-amber-200/30"
                  type="button"
                  @click="selectedActorId = actor.id; selectedCellKey = actor.x !== null && actor.y !== null ? `${actor.x}-${actor.y}` : null"
                >
                  <img
                    v-if="actor.image_url"
                    :src="actor.image_url"
                    :alt="actor.name"
                    class="h-14 w-11 rounded-xl border border-white/10 object-cover object-top"
                  >
                  <div
                    v-else
                    class="flex h-14 w-11 items-center justify-center rounded-xl border border-white/10 bg-white/10 text-sm font-semibold text-amber-100"
                  >
                    {{ actor.name.slice(0, 1) }}
                  </div>
                  <div class="min-w-0">
            ...
            <section class="rounded-[1.5rem] border border-amber-200/10 bg-slate-950/30 p-4 text-sm leading-6 text-slate-300">
              Клик по персонажу выбирает его. Следующий клик по свободной клетке отправляет ход на backend, а затем проигрывает анимацию перемещения на поле.
            </section>
          </div>
        </aside>
      </div>
  ...
  <div
    v-if="isClientReady && isEncounterStartModalOpen && runtimeScene"
    class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/72 px-4 backdrop-blur-sm"
    @click.self="closeEncounterStartModal()"
  >
    <div class="w-full max-w-2xl rounded-[2rem] border border-amber-200/10 bg-slate-950 p-6 shadow-[0_30px_100px_rgba(2,6,23,0.7)]">
      <div class="flex items-start justify-between gap-4">
        <div>
          <p class="text-xs uppercase tracking-[0.2em] text-amber-200/50">Сражение</p>
          <h2 class="mt-2 font-display text-3xl text-amber-50">Выбери участников боя</h2>
          <p class="mt-3 text-sm leading-6 text-slate-300">
            По умолчанию выбраны все акторы на сцене. После запуска будет определена очередность хода.
          </p>
        </div>
        <button
          class="rounded-full border border-amber-200/10 bg-white/5 px-4 py-2 text-sm text-slate-300 transition hover:border-amber-200/20"
          type="button"
          @click="closeEncounterStartModal()"
        >
          Закрыть
        </button>
      </div>

      <div class="mt-6 grid max-h-[55vh] gap-3 overflow-y-auto pr-1">
        <button
          v-for="actor in runtimeScene.actor_instances"
          :key="actor.id"
          class="flex items-center gap-4 rounded-[1.35rem] border p-4 text-left transition"
          :class="selectedEncounterActorIds.includes(actor.id) ? 'border-amber-300/35 bg-amber-300/10' : 'border-amber-200/10 bg-white/5 hover:border-amber-200/20'"
          type="button"
          @click="toggleEncounterActor(actor.id)"
        >
          <img
            v-if="actor.image_url"
            :src="actor.image_url"
      ...
      <div class="mt-6 flex justify-end">
        <button
          class="rounded-full border border-amber-300/30 bg-amber-300/10 px-5 py-3 text-sm text-amber-50 transition hover:border-amber-200/40 disabled:cursor-not-allowed disabled:opacity-50"
          :disabled="selectedEncounterActorIds.length === 0 || isEncounterStarting"
          type="button"
          @click="handleStartEncounter()"
        >
          {{ isEncounterStarting ? 'Запуск...' : 'Запустить сражение' }}
        </button>
      </div>
    </div>
  </div>
