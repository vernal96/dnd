...
<template>
  <main class="relative isolate min-h-screen overflow-hidden px-4 py-6 sm:px-6">
    <div
        class="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(255,229,184,0.65),transparent_30%),linear-gradient(180deg,#f7ecd9_0%,#ead6b3_45%,#dcc29b_100%)]"/>
    <div
        class="pointer-events-none absolute inset-0 opacity-40 [background-image:radial-gradient(circle_at_1px_1px,rgba(120,75,19,0.18)_1px,transparent_0)] [background-size:26px_26px]"/>

    <div :class="immersive ? 'relative w-full space-y-5' : 'relative mx-auto max-w-6xl space-y-5'">
      <header
          class="flex flex-col gap-4 rounded-[1.75rem] border border-amber-950/10 bg-white/40 px-5 py-4 shadow-[0_30px_70px_rgba(82,48,15,0.12)] backdrop-blur-sm sm:flex-row sm:items-center sm:justify-between">
        <div class="flex items-center gap-4">
          <div
              class="flex h-14 w-14 items-center justify-center rounded-[1.2rem] border border-amber-900/15 bg-[radial-gradient(circle_at_top,rgba(248,216,155,0.75),rgba(78,43,20,0.12)_70%)]">
